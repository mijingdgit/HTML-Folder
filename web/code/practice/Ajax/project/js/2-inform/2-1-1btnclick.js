// TODO验证码验证
//初始化验证码图像
function codes(_codeArr, _code, _btn, _textCard1, _passWord1,_carder,_passer, _codeerr, _hint) {
  axios
    .post(
    ...
    .then((res) => {
      ...
      _btn.addEventListener("click", function () {
        // 利用正则让用户填写正确的卡号
        let card = /^[1-9]{1}\d{11}$/;
        let cardValue = _textCard1.value;
        ...
        // 利用正则让用户填写正确的密码
        let pass = /^\d{6}$/;
        let passVaule = _passWord1.value;
        ...
        if (_code.value == res.data.code) {
          ...
          let accountNum = {
            //   验证卡号是否在数据库中
            CardNum: Number(_textCard1.value),
            // 验证密码是否在数据库中并且匹配
            Pwd: Number(_passWord1.value),
          };
          axios
            .post(
            ...
            .then((res) => {
              ...
              if (res.data.state === 1) {
                window.localStorage.setItem("cardNum1", _textCard1.value);
                window.localStorage.setItem("passNum1", _passWord1.value);
